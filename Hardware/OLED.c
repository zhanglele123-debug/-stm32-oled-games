...
//	for (i = 0; i < 16; i++)
//	{
//		OLED_WriteData(OLED_Character[num][i]);			//显示上半部分内容
//	}
//	OLED_SetCursor((Line - 1) * 2 + 1, (Column - 1) * 16);	//设置光标位置在下半部分
//	for (i = 0; i < 16; i++)
//	{
//		OLED_WriteData(OLED_Character[num][i + 16]);		//显示下半部分内容
//	}
//}
//Column 列位置，范围：1~16
void OLED_ShowCharacter(uint8_t Line, uint8_t Column, short num)
...
{      	
	...
	for (i = 0; i < 8; i++) {
		OLED_WriteData(~OLED_F8x16[Char - ' '][i]); // 数据取反
	}
	OLED_SetCursor((Line - 1) * 2 + 1, (Column - 1) * 8);
	for (i = 0; i < 8; i++) {
		OLED_WriteData(~OLED_F8x16[Char - ' '][i + 8]); // 数据取反
	}
}
...
void OLED_ShowString_Reverse(uint8_t Line, uint8_t Column, char *String)
{
	uint8_t i;
	for (i = 0; String[i] != '\0'; i++) {
		OLED_ShowChar_Reverse(Line, Column + i, String[i]);
	}
}
...
void OLED_ShowNum_Reverse(uint8_t Line, uint8_t Column, uint32_t Number, uint8_t Length)
{
	uint8_t i;
	for (i = 0; i < Length; i++) {
		OLED_ShowChar_Reverse(Line, Column + i, 
...
void OLED_ShowSignedNum_Reverse(uint8_t Line, uint8_t Column, int32_t Number, uint8_t Length)
{
	...
	if (Number >= 0)
	{
		OLED_ShowChar_Reverse(Line, Column, '+');
		Number1 = Number;
	}
	else
	{
	...
	for (i = 0; i < Length; i++) {
		OLED_ShowChar_Reverse(Line, Column + i + 1, 
...
void OLED_ShowHexNum_Reverse(uint8_t Line, uint8_t Column, uint32_t Number, uint8_t Length)
{
	...
	for (i = 0; i < Length; i++) {
		SingleNumber = Number / OLED_Pow(16, Length - i - 1) % 16;
		if (SingleNumber < 10) {
			OLED_ShowChar_Reverse(Line, Column + i, SingleNumber + '0');
		} else {
			OLED_ShowChar_Reverse(Line, Column + i, SingleNumber - 10 + 'A');
		}
	}
}
...
{      	
	...
	OLED_SetCursor((Line - 1) * 2, (Column - 1) * 8);
	for (i = 0; i < 16; i++) {
		OLED_WriteData(~OLED_Character[num][i]); // 数据取反
	}
	OLED_SetCursor((Line - 1) * 2 + 1, (Column - 1) * 8);
	for (i = 0; i < 16; i++) {
		OLED_WriteData(~OLED_Character[num][i + 16]); // 数据取反
	}
}
...
{
	// 计算起始位置（像素坐标）
	uint8_t startX = (Column - 1) * 8;
	uint8_t startY = (Line - 1) * 2;
	
	// 计算结束位置（像素坐标）
	uint8_t endX = startX + (Width * 8) - 1;
	uint8_t endY = startY + (Height * 2) - 1;
